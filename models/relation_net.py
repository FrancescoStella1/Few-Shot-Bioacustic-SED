...
class RelationNet(nn.Module):
    def __init__(self, embedding_model: nn.Module, bn_momentum: float, dropout_1: float, dropout_2: float, dropout_3: float,
                 dropout_4: float, kernel_size_1: int, kernel_size_2: int, kernel_size_3, kernel_size_4: int,
                 out_channels_1: int, out_channels_2: int, out_features_1: int, out_features_2: int):
        super(RelationNet, self).__init__()
        self.f = embedding_model
        in_channels = embedding_model._modules["2"]._modules["0"].out_channels * 2
        self.g = nn.Sequential(
            ...
            nn.MaxPool2d(kernel_size=kernel_size_4),
            nn.Flatten(),
            nn.LazyLinear(out_features=out_features_1),
            nn.BatchNorm1d(out_features_1, momentum=bn_momentum),
            nn.PReLU(),
            nn.Dropout(dropout_3),
            nn.Linear(in_features=out_features_1, out_features=out_features_2),
            nn.BatchNorm1d(out_features_2, momentum=bn_momentum),
            nn.PReLU(),
            nn.Dropout(dropout_4),
            nn.Linear(in_features=out_features_2, out_features=1),
            nn.Sigmoid()
        )

    def forward(self, support, query):
        # Compute embeddings
        support = self.f(support)
        query = self.f(query)
        # print("Shape after embedding function: ", support.shape)

        embed = torch.cat((support, query), dim=1)
        # print("Shape of concatenated embeddings: ", embed.shape)
        
        out = self.g(embed)
        # print("output shape: ", out.shape)
        return torch.squeeze(out, dim=1)  # , support, query
    
    
class RelationDataset(Dataset):
    def __init__(self, filepath: str, pipeline: CustomPipeline = None):
        """
        Creates a datasets providing pairs for training Relation Network.
...
def register_hooks(model: RelationNet):
    """
...
def get_dataloader(split: str) -> DataLoader:
    ...
    dataset = RelationDataset(filepath=filepath, pipeline=pipeline)
...
def load_model(load_finetuned: bool = False) -> RelationNet:
    """
    ...
    """
    device = "cuda" if torch.cuda.is_available() else "cpu"
    model_name = "relation_network_finetuned.pt" if load_finetuned else "relation_network.pt"
...
def create_model(embedding_model: torch.nn.Module) -> RelationNet:
    ...
    model = RelationNet(embedding_model=embedding_model, bn_momentum=1, dropout_1=0.3, dropout_2=0.2,
                        dropout_3=0.2, dropout_4=0.1, kernel_size_1=2, kernel_size_2=3, kernel_size_3=4, kernel_size_4=6,
                        out_channels_1=128, out_channels_2=256, out_features_1=128, out_features_2=32)
    return model

def create_model_from_trial(embedding_model: torch.nn.Module) -> RelationNet:
    try:
        fpath = os.path.join("rnet_trials", CONFIG.get("OPTIMIZATION", "rnet_trial"), "result.json")
        with open(fpath, "r") as f:
            params = json.load(f)["config"]
        model = RelationNet(embedding_model=embedding_model, bn_momentum=params["bn_momentum"], dropout_1=params["dropout_1"],
                            dropout_2=params["dropout_2"], dropout_3=params["dropout_3"], dropout_4=params["dropout_4"],
                            kernel_size_1=params["kernel_size_1"], kernel_size_2=params["kernel_size_2"], kernel_size_3=params["kernel_size_3"],
                            kernel_size_4=params["kernel_size_4"], out_channels_1=params["out_channels_1"], out_channels_2=params["out_channels_2"],
                            out_features_1=params["out_features_1"], out_features_2=params["out_features_2"])
        return model
...
def save_model(epoch: int, model: RelationNet, optimizer: torch.optim.Optimizer,
               filename: str):
    """
...
def finetune_rnet(split: str):
    ...
    embedding_model = get_embedding_model(flatten=False)
    if CONFIG.get("MODELS", "freeze_embedding_module") == "true":
        print("Freezing embedding module parameters...")
        for _, param in embedding_model.named_parameters():
            param.requires_grad = False
    ...
    for epoch in tqdm(range(epochs), desc=f"Finetuning on {split}",
        ...
        for images, label_ids, _ in tqdm(iter(dataloader), desc="Finetuning",
            ...
            predictions = model(anchor, query)
        ...
        if best_loss is None or avg_loss <= (best_loss - delta_loss):
            ...
            save_model(epoch=epoch, model=model, optimizer=optimizer,
                       filename=os.path.join(save_path, "relation_network_finetuned.pt"))
        else:
            patience -= 1
    ...
    hist.to_csv(os.path.join(save_path, "rnet_finetuning_history.csv"), index=False,
                             columns=hist.columns)


def eval_rnet():
    """
    Evaluates Relation Network on Validation set.
    """
    device = "cuda" if torch.cuda.is_available() else "cpu"
    embedding_model = get_embedding_model(flatten=False)
    model = create_model(embedding_model=embedding_model)
    ...
    with torch.no_grad():
        ...
        plt.title("ROC curve for Relation Network")
        plt.show()
...
def train_rnet(model: RelationNet, optimizer: torch.optim.Optimizer, epochs: int) -> RelationNet:
    """
    Performs the training of the Relation Network for model selection.

    Parameters
    ----------
    model: RelationNet
        RelationNetwork instance.
    ...
    for epoch in range(epochs):
        ...
        for (support_images, query_images), _, (support_labels, query_labels) in iter(dataloader):
            optimizer.zero_grad()
            predictions = model(support_images[0].to(device), query_images[0].to(device))
            support_labels = np.array(support_labels).squeeze(axis=1)
            query_labels = np.array(query_labels).squeeze(axis=1)
            targets = np.zeros(support_labels.shape[0])
            idxs = np.where(support_labels == query_labels)[0]
            targets[idxs] = 1.
            targets = torch.tensor(targets, device=device, dtype=torch.float)
            loss = loss_fn(predictions, targets)
            loss.backward()
            optimizer.step()
...
def train_rnet_main() -> pd.DataFrame:
    """
    Performs the training of the Relation Network.
    ...
    # Define number of epochs, patience and delta loss for patience
    epochs = CONFIG.getint("TRAINING", "epochs")
    original_patience = CONFIG.getint("TRAINING", "patience")
    patience = original_patience
    delta_loss = CONFIG.getfloat("TRAINING", "delta_loss")    
    best_loss = None

    # Define savepath and dataframes for training and validation history
    save_path = CONFIG.get("MODELS", "save_path")
    ...
    # Dataloaders
    train_dataloader = get_dataloader(split="training")
    val_dataloader = get_dataloader(split="val_train")
    ...
    model = model.to(device)
    weight_decay = 0.028
    optimizer = torch.optim.NAdam(params=model_params, lr=1e-4, weight_decay=weight_decay)
    loss_fn = torch.nn.BCELoss(reduction="mean")  # torch.nn.MSELoss(reduction="mean")
    ...
    for epoch in tqdm(range(epochs), desc="Epoch", total=epochs):
        ...
        model = model.train()
        for (support_images, query_images), _, (support_labels, query_labels) in tqdm(iter(train_dataloader),
                                                                                      desc="Training episode", total=len(train_dataloader)):
            optimizer.zero_grad()
            predictions = model(support_images[0].to(device), query_images[0].to(device))
            support_labels = np.array(support_labels).squeeze(axis=1)
            query_labels = np.array(query_labels).squeeze(axis=1)
            ...
            loss.backward()
            optimizer.step()
            train_acc.update(predictions, targets)
            train_f1.update(predictions, targets)
            train_prec.update(predictions, targets)
            avg_train_loss += loss.item()
        avg_train_loss /= len(train_dataloader)
        print(f"Training stats for epoch: {epoch+1}\n")
        ...
        model = model.eval()
        with torch.no_grad():
            for (support_images, query_images), _, (support_labels, query_labels) in tqdm(iter(val_dataloader),
                                                                                          desc="Validation episode", total=len(val_dataloader)):
                predictions = model(support_images[0].to(device), query_images[0].to(device))
                support_labels = np.array(support_labels).squeeze(axis=1)
                query_labels = np.array(query_labels).squeeze(axis=1)
                targets = np.zeros(support_labels.shape[0])
        ...
        if best_loss is None or avg_val_loss <= (best_loss - delta_loss):
            ...
            patience = original_patience
            save_model(epoch=epoch, model=model, optimizer=optimizer, loss=loss_fn, 
                        filename=os.path.join(save_path, "relation_network.pt"))
        else:
            patience -= 1
    ...
    hist.to_csv(os.path.join(save_path, "rnet_training_history.csv"), index=False,
                            columns=hist.columns)
    return hist
